...
export const rootReducer = function (state: AppState = initialState, action: TodoActionsUnion) {
    switch (action.type) {
        ...
        case TodoActionTypes.SAVE_TASK_SUCCESS: {
            let stateCopy = createCopy(state);
        ...
        case TodoActionTypes.SET_ACTIVE_PERSON_INDEX: {
            let stateCopy = createCopy(state);
            stateCopy.activePersonIndex = stateCopy.persons.findIndex(p => p.id === action.payload);
            return { ...stateCopy };
        }
        case TodoActionTypes.ADD_PERSON: {
            let stateCopy = createCopy(state);
            stateCopy.persons = [...stateCopy.persons, { title: "", id: action.payload } ];
        ...
        case TodoActionTypes.SAVE_PERSON: {
            let stateCopy = createCopy(state);
            let person = stateCopy.persons[action.payload.index];
            person.id = person.id || Guid.create().toString();
            person.title = action.payload.title;
            return { ...stateCopy };
        }
        case TodoActionTypes.SAVE_PERSON_SUCCESS: {
            let stateCopy = createCopy(state);
